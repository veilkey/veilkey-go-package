...
// ReadPasswordFromFileEnv reads a password from the file path in VEILKEY_PASSWORD_FILE.
// Returns empty string if the env var is not set. Fatals on read errors or empty file.
...
// ReadPassword reads a password from TTY (interactive) or stdin (piped).
...
// ParseDurationEnv parses a time.Duration from an environment variable.
// Returns defaultVal if the variable is unset or invalid.
